...
```{r}
# cohort$baseline_quintile <- factor(cohort$baseline_quintile, labels = seq(5, 1))
# cohort$quintile <- factor(cohort$quintile, labels = seq(5, 1))
# cohort$quantile <- factor(cohort$quantile, labels = seq(4, 1))
...
clean.names <- c(
  ...
  "Age",
  "Male",
  "Baseline <10 & Increase <=10",
  "Baseline <20 & Increase <=5",
  "Baseline <20 & Increase <=10",
  "Baseline <30 & Increase <=5",
  "Baseline <30 & Increase <=10"
)
...
ref.category <- data.frame(row.names = "Baseline <10 & Increase <=5", or = 1, lower = 1, upper = 1)
...
color.schema <- c("#0D0D0D", "#8C0303", "#3D7345", "#BB7A32", "#DDB247", "#4B7210", "#519188", "#14312A", "#653040")
```
